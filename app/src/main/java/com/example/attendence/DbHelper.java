...
public class DbHelper extends SQLiteOpenHelper {
   ...
   //student table
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        //Log.d("check","Query beung run!!"+CREATE_CLASS_TABLE);
    }
   ...
   long addClass(String className,String subjectName){
    ...
    String getStatus(long sid,String date){
        ...
        if(cursor.moveToFirst()){
            int status_key = cursor.getColumnIndex(STATUS_KEY);
            status = cursor.getString(status_key);
        }
        return status;
    }
    Cursor getDistinctMonths(long cid){
        SQLiteDatabase database = this.getReadableDatabase();
        return database.query(STATUS_TABLE_NAME,new String[]{DATE_KEY},C_ID+"="+cid,null,"substr("+DATE_KEY+",4,7)",null,null);//0
    }
